...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/01 18:08:33 by cyuriko           #+#    #+#             */
...
static void		init_params_ship(t_graphon *graphon/*, t_window *window*/, t_fractol *fractol)
{
...
static void		run_ship(t_graphon *graphon, t_window *window)
{
...
void	go_ship(t_window *window, t_graphon *graphon, t_fractol *fractol)
{
	...
	graphon->program_string = read_file(
			"/Users/cyuriko/fractol_to_git/fract_codes/burning_ship.cl");////////////читаем файл с кернелом в програм стринг
	...
	error(graphon->ret);
	init_params_ship(graphon/*, window*/, fractol);
	run_ship(graphon, window/*, fractol*/);
}


void	draw_ship(t_graphon *graphon, t_fractol *fractol, t_window *window)
{
	init_params_ship(graphon, fractol);
	run_ship(graphon, window);
}